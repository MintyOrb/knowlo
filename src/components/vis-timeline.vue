// this is lightly modified from https://github.com/r3code/vue-vis-network/
<template>
...
export default defineComponent({
  ...
  watch: { 
    items: function(items) {
        console.log(items)
      this.timelineData.items = new DataSet(items)
      this.updateTimeline()
    },
  ...
  methods: {
    ...
    initTimeline: function () {
      ...
      // Create a Timeline
      if (!this.timeline) {
        this.timeline = new Timeline(container, this.timelineData.items, this.timelineData.groups, {});
...
.vs-btn-group>.vs-btn-group:not(:last-child)>.vs-btn {
  border-top-right-radius: 0;
  border-bottom-right-radius: 0;
}

.vs-btn-group>.vs-btn:not(:first-child),
.vs-btn-group>.vs-btn-group:not(:first-child)>.vs-btn {
...
.vis-itemset .vis-foreground .vis-group:first-child {
